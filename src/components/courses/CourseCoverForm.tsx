...
export function CourseCoverForm({ courseId, coverImagePath }: { courseId: string; coverImagePath: string | null }) {
  ...
  return (
    <section className="max-w-xl rounded-xl border border-zinc-200 bg-white p-4">
      <h2 className="text-sm font-medium text-zinc-900">Cover image</h2>
      <p className="mt-0.5 text-xs text-zinc-500">
        Shown on your students&rsquo; course cards and as the banner on the course page. Landscape works best
        (around 1200 × 675). JPG, PNG or WebP, up to 3 MB.
      </p>
      ...
      <div className="mt-3 aspect-[16/9] w-full overflow-hidden rounded-lg border border-dashed border-zinc-300 bg-zinc-50">
        {current ? (
          ...
          <img src={current} alt="Course cover" className="h-full w-full object-cover" />
        ) : (
          <div className="grid h-full place-items-center text-xs text-zinc-400">No cover yet</div>
        )}
      </div>
      ...
      <form
        ...
        onSubmit={(e) => {
          ...
          if (file && file.size > MAX_BYTES) {
            e.preventDefault();
            setClientError("That image is over 3 MB — try a smaller or compressed one.");
          }
        }}
      ...
      {coverImagePath && !preview && (
        <form action={removeCourseCover.bind(null, courseId)} className="mt-2">
          <SubmitButton pendingLabel="Removing…" className="text-xs text-red-600 hover:underline disabled:opacity-50">
            Remove cover
          </SubmitButton>
        </form>
      )}
